...
def generate_media_thumbnail(
    ...
    *,
    ...
    """Best-effort local thumbnail for media that arrived with none.

    Every delivery path funnels through this as a last-resort fallback so
    a downloaded, leeched, or mirrored file never reaches Telegram with a
    blank thumbnail just because its source didn't supply one of its own
    (a torrent has no metadata to draw one from, a forwarded Telegram
    message may carry no embedded preview, and even a yt-dlp extractor
    occasionally omits thumbnail info). For video this grabs a real frame
    a few seconds in (skipping a possibly-black opening frame at 0:00,
    with a second attempt at 0:00 for very short clips); for audio it
    pulls any cover art embedded in the file's tags. Mirrors the size/
    quality handling already used for downloaded thumbnail URLs so the
    result stays under Telegram's ~200 KB thumbnail cap.

    Requires ffmpeg. Returns None (never raises) if ffmpeg is missing, the
    source has nothing to grab (e.g. an audio file with no embedded
    artwork), or conversion otherwise fails -- a missing thumbnail must
    never block delivering the media itself.
    """
    ...
    if audio_only:
        attempts = [["-i", str(source_path), "-an", "-vcodec", "mjpeg"]]
    else:
        attempts = [
            ["-ss", "3", "-i", str(source_path)],
        ...
        for input_args in attempts:
            for quality in (8, 15):
                ...
                if (
                    ...
                    and target.exists()
                    and 0 < target.stat().st_size <= 200_000
                ):
                    target.chmod(0o600)
                    return target
                target.unlink(missing_ok=True)
...
def probe_has_video_stream(path: Path) -> bool | None:
    """Check whether a local media file actually contains a video stream.

    Used to confirm the true audio/video nature of a file (a replied
    Telegram attachment, for example) instead of relying only on its
    filename extension or the mime type Telegram reports -- either of
    which can be missing, generic (``application/octet-stream``), or
    simply wrong. Returns True/False when ffprobe is able to answer, or
    None when ffprobe is unavailable or the probe itself fails; callers
    should fall back to their own heuristic in that case rather than
    treating None as a definite "no video".

    Embedded cover art (an MP3/FLAC/M4A tag image) is stored as an
    "attached picture" video stream and would otherwise make an audio
    file with album art look like it has video; those streams are
    explicitly excluded so only a real playable video track counts.
    """
...
class YTDLPDownloader:
    ...
    async def search(
        ...
        try:
            ...
            # yt-dlp rewrites the cookiefile it was given with any rotated
            # session cookies before this materialized copy is discarded.
            # Persist that back into the encrypted store first, or stored
            # YouTube cookies never advance past their original snapshot.
            self.cookie_store.sync_back(user_id, cookie_path)
    ...
    async def download(
        ...
        try:
            ...
            # See the matching comment in search(): persist any cookiejar
            # rotation yt-dlp wrote back before discarding the temp copy.
            self.cookie_store.sync_back(user_id, cookie_path)
            self.cookie_store.cleanup_materialized(cookie_path)

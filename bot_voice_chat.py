...
class VoiceChatController:
    ...
    @staticmethod
    def _ffmpeg_params(url: str, seconds: int = 0) -> str | None:
        """Build the FFmpeg input flags PyTgCalls applies to a stream.

        Two things live here:

        * ``-ss`` for a resume/seek offset (input-side seek: fast, and
          already how this was done before this method existed).
        * FFmpeg's ``-reconnect*`` family for remote HTTP(S) sources. These
          make FFmpeg transparently re-open a stalled/dropped connection
          instead of the read simply failing, which is the single biggest
          cause of playback freezing or stopping outright on a flaky
          connection to a signed googlevideo/CDN URL. They are protocol
          options that only apply to the http/https demuxer, so they are
          skipped for local file paths (downloaded Telegram media) where
    ...
    def _audio_stream(track: VoiceTrack, seconds: int = 0) -> MediaStream:
        ...
        return MediaStream(
            ...
            ffmpeg_parameters=VoiceChatController._ffmpeg_params(audio_path, seconds),
        )
    ...
    def _video_stream(track: VoiceTrack, seconds: int = 0) -> MediaStream:
        ...
        return MediaStream(
            ...
            ffmpeg_parameters=VoiceChatController._ffmpeg_params(video_path, seconds),
        )
        ...
        *,
        ...
        def extract() -> dict[str, Any]:
            ...
            try:
                ...
                if self._cookie_store:
                    # Persist any cookiejar rotation yt-dlp wrote back into
                    # this materialized file before it's discarded, so
                    # voice-chat playback keeps YouTube cookies fresh the
                    # same way downloads/search do.
                    self._cookie_store.sync_back(requester_id, cookie_path)
        ...
        if video and not video_url:
            # A "video" request that resolves to an audio-only source (an
            # audio-hosting link, or a video platform temporarily serving
            # only an audio format) must not hard-fail when a perfectly
            # playable audio stream was found. Downgrade to audio-only
            # instead of raising, so a wrong video/audio guess never
            # breaks playback -- it only means no video track.
            video = False
    ...
    def _probe_public_media_url(
        ...
        try:
            ...
            if not cls._is_playable_content_type(content_type, filename):
                return None
            # A "video" request against a source that turns out to be
            # audio-only (Content-Type: audio/..., or an audio file
            # extension) is still played -- just as audio -- instead of
            # being rejected here, matching the same graceful downgrade
            # ``_resolve`` applies on the slower yt-dlp extraction path.
            resolved_video = video and cls._content_type_has_video(
            ...
            return VoiceTrack(
                ...
                audio_url=current,
                video_url=current if resolved_video else None,
            )
    ...
    def _is_playable_content_type(
        ...
        """Return whether a probed URL looks like playable media at all.

        This intentionally does not consider the requested audio/video mode
        -- a source that turns out to be audio-only should still be
        accepted (and simply played as audio) rather than rejected here,
        which would otherwise force a much slower yt-dlp round-trip only to
        reach the same conclusion. See ``_content_type_has_video`` for the
        separate, mode-aware question of whether it also carries video.
        """
    ...
    @staticmethod
    def _content_type_has_video(content_type: str, filename: str = "") -> bool:
        """Best-effort guess at whether a probed URL actually carries video."""
        ...
        if content_type in {
            ...
            # HLS/DASH manifests commonly carry video; treat as video-capable.
            return True
        video_extensions = {
            ".3g2", ".3gp", ".avi", ".flv", ".m2ts", ".m4v", ".mkv",
            ".mov", ".mp4", ".mpeg", ".mpg", ".m3u", ".m3u8", ".mpd",
    ...
    def _direct_stream_fallback(
        ...
        *,
        ...
        if Path(path).suffix.lower() in audio_only_extensions:
            # An unambiguous audio extension settles this immediately: play
            # it as audio even if "video" was requested, rather than
            # bailing out here and forcing a slower network probe or a full
            # yt-dlp round-trip only to reach the same conclusion.
            video = False

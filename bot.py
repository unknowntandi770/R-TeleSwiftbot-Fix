...
This file replaces main.py and provides:
  ...
  • Bug-fix patches applied before the bot loads
      – URLError import fix in bot_downloader
      – from_user None guard in delete_cookies handler
      – enable_redis default corrected to False
      – shared variable initialised before the except block in _share_file
      – double user_id assignment cleaned up in store_search_command
...
# ─── 3. Bug-fix patches (applied before importing bot_main) ─────────────────


def _apply_patches() -> None:
    """
    Monkey-patch fixes for bugs found in the source modules.
    Each patch is self-contained and well-commented so it is easy to remove
    once the underlying module is fixed in the canonical source.
    """

    # ── PATCH 1 ──────────────────────────────────────────────────────────────
    # bot_downloader.py uses `URLError` in _probe_source_kind() but only
    # imports `HTTPError` from urllib.error.  This causes a NameError at
    # runtime whenever an unknown-host URL is probed.
    # Fix: inject the import into the module's namespace after import.
    try:
        import bot_downloader
        from urllib.error import URLError

        bot_downloader.URLError = URLError  # type: ignore[attr-defined]
        logger.debug("PATCH 1 applied: URLError injected into bot_downloader")
    except (ImportError, AttributeError):
        logger.exception("PATCH 1 failed: could not import bot_downloader")

    # ── PATCH 2 ──────────────────────────────────────────────────────────────
    # bot_config.py defaults enable_redis to True (_as_bool(..., "true")) but
    # documentation and sample_config.env say the default is false.
    # A Redis-less deployment silently fails on every startup.
    # Fix: if the user has not explicitly set ENABLE_REDIS, default to false.
    if "ENABLE_REDIS" not in os.environ:
        os.environ["ENABLE_REDIS"] = "false"
        logger.debug("PATCH 2 applied: ENABLE_REDIS defaulted to false")

    # The source supports legacy names, but the enhanced preflight also uses
    # them. Normalize them once so Settings and the rest of the bot see one
    ...
    # Repair the old empty-cookie-key fallback with an exclusive create. The
    # old O_TRUNC-only implementation could race and overwrite another
    # process's key during first startup.
    try:
        import bot_config

        def _safe_rewrite_empty_cookie_key(key_path: Path, value: str) -> str:
            ...
            try:
                ...
                if existing:
                    ...
                    return existing
                # A competing writer left an empty file. Replace it only after
                # confirming it is still empty and do so atomically.
        ...
        bot_config._rewrite_empty_cookie_key = _safe_rewrite_empty_cookie_key
        logger.debug("PATCH 5 applied: cookie-key fallback uses exclusive create")
    except (ImportError, OSError):
        logger.exception("PATCH 5 failed: cookie-key fallback remains unchanged")

    # ── PATCH 3 ──────────────────────────────────────────────────────────────
    # bot_main.Bot.delete_cookies calls message.from_user.id without checking
    # whether from_user is None (channel messages can have no sender).

    # ── PATCH 4 ──────────────────────────────────────────────────────────────
    # The archive already contains the safe shared=None initialization.  The
    # compatibility wrapper below remains defensive for older uploaded ZIPs.

    logger.debug("Core environment patches applied")
...
# ─── 6. Additional patched handler methods ───────────────────────────────────

async def _patched_delete_cookies(self: Bot, _client, message: Message) -> None:
    """
    PATCH 3: Guard against message.from_user being None.
    Original code would AttributeError in channels/forwarded messages.
    """
    ...
    deleted = self.cookies.delete(user_id)
    if deleted:
        await message.reply_text(
            "🗑 <b>Cookies deleted</b>\n\n"
            "Your stored browser cookies have been securely removed.\n"
            "You can upload new ones any time with /cookies."
        )
    else:
        await message.reply_text(
            "📭 <b>No cookies found</b>\n\n"
            "There are no stored cookies associated with your account."
        )


async def _patched_share_file(
    self: Bot,
    message: Message,
    *,
    ...
    """
    PATCH 4: Ensure `shared` is initialised before the except block.
    Original _share_file references `shared` in the except handler even
    when file_links.add_media() itself raised before assigning it.
    """
    # Delegate to the original but wrap the NameError defensively.
    # The real fix is in the underlying method — this wrapper logs the
    # mismatch and ensures the user sees a clean error message.
    try:
        await _original_share_file(
            self,
            message,
            owner_id=owner_id,
        ...
        if "shared" in str(exc):
            logger.warning("Caught patched NameError in _share_file: %s", exc)
            try:
                await message.reply_text(
                    "❌ <b>Could not create file link</b>\n\n"
                    "The file-link service encountered an internal error. "
                    "Please try /filestream again."
                )
            except Exception:
                pass
        else:
...
# Save original so the wrapper can call it
_original_share_file: Callable | None = None


# ─── 7. Rate-limit wrappers for Bot methods ───────────────────────────────────
...
# ─── 8. New command handlers (injected at runtime) ───────────────────────────
...
# ─── 9. Inject new methods and patches into Bot ───────────────────────────────

def _patch_bot_class() -> None:
    global _original_share_file

    if getattr(Bot, "_enhanced_bot_patched", False):
        return

    # Patch 3: safe delete_cookies
    if hasattr(Bot, "delete_cookies"):
        Bot.delete_cookies = _patched_delete_cookies  # type: ignore[method-assign]
        logger.debug("PATCH 3 applied: delete_cookies guarded for None from_user")

    # Patch 4: safe _share_file wrapper
    if hasattr(Bot, "_share_file"):
        _original_share_file = Bot._share_file  # type: ignore[attr-defined]
        Bot._share_file = _patched_share_file    # type: ignore[method-assign]
        logger.debug("PATCH 4 applied: _share_file NameError guarded")
...
# ─── 10. Wire new commands into the Pyrogram app on startup ──────────────────
...
# ─── 14. Enhanced run() wrapping bot_main.Bot.run() ─────────────────────────
...
# ─── 15. Signal handling + main entry point ───────────────────────────────────

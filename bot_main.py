...
class Bot:
    ...
    def _infer_vplay_mode(mode: str, source: str) -> str:
        """Default a bare media URL to video unless it looks audio-only.

        ``/vplay`` historically defaulted to audio for search text (kept
        below, unchanged). For an explicit URL, though, that default used
        to only get corrected for a short YouTube/Drive/manifest allow-
        list -- a bare link to any other video site (Vimeo, X/Twitter,
        TikTok, Instagram, Facebook, Twitch, Dailymotion, Reddit, and the
        hundreds of others yt-dlp supports) would silently drop its video
        track and play audio-only with no explanation. Rather than chase
        an ever-growing list of video hosts, this assumes video for any
        explicit URL and only steps back to audio for the handful of
        audio-first hosts/extensions below. A wrong guess here is cheap:
        the voice-chat resolver gracefully falls back to audio-only
        playback if a source turns out to have no video track, so
        defaulting to video only affects whether video is *attempted*,
        never whether playback succeeds. Explicit audio/video keywords in
        the command always win over this inference.
        """
        ...
        if host in audio_first_hosts or any(
            host.endswith(f".{domain}") for domain in audio_first_hosts
        ):
            return mode
        audio_extensions = {
            ".aac",
        ...
        if Path(parsed.path).suffix.lower() in audio_extensions:
            return mode
        return "video"
    ...
    @staticmethod
    def _vplay_invoked_alias(command_text: str) -> str:
        """Return which command word was actually typed: vplay or vcplay.

        Both are registered on the same handler. ``/vcplay`` is the
        "just play it" alias: it always auto-selects the best available
        quality instead of stopping for the quality-chooser prompt, while
        ``/vplay`` keeps its existing interactive picker for video.
        """
        match = re.match(r"^/([A-Za-z0-9_]+)", command_text.strip())
        return match.group(1).lower() if match else "vplay"
        ...
        *,
        ...
        if not source and not replied_media:
            await message.reply_text(
                ...
                "Prefer not to be asked? <code>/vcplay</code> is the same command "
                "but always auto-streams at the best available quality "
                "immediately, no chooser.\n\n"
                "The group video chat starts automatically when playback begins."
            )
            return
        auto_best_quality = self._vplay_invoked_alias(command_text) == "vcplay"
        needs_quality = (
            stream_quality is None
            and not auto_best_quality
            and (
                mode == "video"
        ...
        try:
            ...
            if restricted_source:
                ...
                # A filename extension or the mime type Telegram reports is
                # only a guess at whether a reply carries video (containers
                # are sometimes ambiguous, and Telegram doesn't always set a
                # precise mime type). Now that the file is on disk, confirm
                # with an actual ffprobe stream inspection when available --
                # this decides both the default mode and whether an explicit
                # "/vplay video" against an audio-only reply is rejected.
                probed_has_video = await asyncio.to_thread(
                    probe_has_video_stream, downloaded_path
                )
                if probed_has_video is None:
                    probed_has_video = (
                        self._replied_media_mode(file_name, mime_type) == "video"
                    )
                if not self._vplay_mode_is_explicit(command_text):
                    mode = "video" if probed_has_video else "audio"
                if mode == "video" and not probed_has_video:
                    raise ValueError(
    ...
    async def vc_seek(self, _: Client, message: Message) -> None:
        ...
        try:
            ...
            await message.reply_text(
                "❌ <b>Please enter a whole number of seconds.</b>\n\n"
                "Example: <code>/vseek 90</code>"
            )
            return
        try:
    ...
    async def vc_volume(self, _: Client, message: Message) -> None:
        ...
        try:
            ...
            await message.reply_text(
                "❌ <b>Please enter a whole number from 1 to 200.</b>\n\n"
                "Example: <code>/vvolume 100</code>"
            )
            return
        try:
    ...
    async def _send_item(
        ...
        *,
        ...
        if not item.thumbnail or not item.thumbnail.exists():
            # Last-resort fallback so nothing reaches Telegram with a blank
            # thumbnail just because its source (a torrent, a forwarded
            # message, an extractor with no thumbnail metadata) didn't
            # supply one. This must run before archiving below: the
            # archived copy is what future cache/file_id-based resends
            # reuse, so a thumbnail generated only after archiving would
            # never reach those. Assigning it back onto the item also lets
            # the existing item.thumbnail cleanup elsewhere pick it up.
            item.thumbnail = await asyncio.to_thread(
                generate_media_thumbnail,
                item.path,
                item.path.with_name(f"{item.path.stem}.generated-thumb.jpg"),
                ...
                await asyncio.wait_for(
                    self.app.set_bot_commands(
                        [
                            ...
                            BotCommand("stats", "Show queue and usage statistics"),

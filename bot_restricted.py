...
_URL_RE = re.compile(
    # telegram.dog is Telegram's own officially documented alternative to
    # t.me/telegram.me (core.telegram.org/api/links) for when the .me TLD
    # is blocked or unavailable, so it gets the same recognition here.
...
def _message_media(message: object) -> tuple[str, int, str | None, bool, object] | None:
    for attribute, fallback_name in (
        ...
        return name or fallback_name, size, mime_type, audio_only, media
    return None
...
class RestrictedMessageDownloader:
        ...
        try:
            for index, message in enumerate(messages, 1):
                ...
                try:
                    ...
                    if len(messages) == 1:
                        # A single-message request keeps its specific error
                        # instead of being folded into a generic batch
                        # summary below.
                        raise
                    # One bad message (expired file reference, a transient
                    # RPC error, a single oversized attachment) must not
                    # cost the rest of an otherwise-successful batch/range
                    # download. Skip it, remember why, and keep going -
                    # mirrors how playlist downloads already tolerate
                    # per-item failures in bot_downloader.py.
                    logger.warning(
                        "Skipping message %s in restricted batch %s: %s",
                        getattr(message, "id", "?"),
                        source.url,
                        exc,
                    )
            ...
            if not items:
                if failures:
                    raise RestrictedContentError(
                        f"None of the {len(messages)} requested messages could "
                        f"be retrieved. Last error: {failures[-1]}"
                    )
                raise RestrictedContentError(
            ...
            return DownloadResult(
                ...
                streamed_failures=len(failures),
            )
    ...
    async def _expand_media_groups(
        ...
        for message in messages:
            ...
            except (RPCError, RestrictedContentError):
                # _retry_telegram converts a persistent FloodWait into a
                # RestrictedContentError rather than an RPCError. Either way,
                # failing to expand one album must not abort the whole
                # request: keep the message we already have and move on.
                continue
    ...
    async def _materialize(
        ...
        *,
        ...
        if media:
            name, declared_size, _mime_type, audio_only, media_obj = media
            ...
            thumbnail = await self._materialize_thumbnail(
                client, media_obj, output_dir, message_id
            )
            return DownloadItem(
                path=path,
                title=title[:180],
    ...
    async def _materialize_thumbnail(
        self,
        client: Client,
        media_obj: object,
        output_dir: Path,
        ...
        """Pull Telegram's own embedded preview image for a video/document/
        animation so items retrieved via /save never ship with a blank
        thumbnail in chat previews. Telegram already generates and stores
        one for most visual media; this only fetches it, it never runs
        ffmpeg or otherwise generates a new one. Best-effort: any failure
        here must not fail the (already-downloaded) media item itself.
        """
        thumbs = getattr(media_obj, "thumbs", None) or []
        if not thumbs:
            return None
        # Telegram returns thumbs smallest-to-largest; the last one is the
        # highest-resolution version that still respects Telegram's own
        # ~200 KB inline-thumbnail size cap.
        file_id = getattr(thumbs[-1], "file_id", None)
        if not file_id:
            return None
        target = output_dir / f"{message_id}.thumbnail.jpg"
        try:
            downloaded = await self._retry_telegram(
                lambda: client.download_media(file_id, file_name=str(target))
            )
        except (RPCError, RestrictedContentError):
            return None
        path = Path(downloaded) if downloaded else target
